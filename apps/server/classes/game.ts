...
import { rooms, setupListeners } from "../setupListeners";
  ...
  setupListeners(socket: Socket) {
    ...
    socket.on("disconnect", () => {
      if (socket.id === this.gameHost) {
        ...
        if (this.players.length !== 0) this.gameHost = this.players[0].id;
        else rooms.delete(this.gameId);
      }
